...
public class EchoServer {
    ...
    private static String cantidad_pagar=new String();
        ...
        try {
            ...
            while(listening) // bucle infinito
            {
                ...
                // revisamos que en el socket el cliente ha enviado algun texto
                 cantidad_pagar=is.readLine();
                 tarjeta=is.readLine();
                 cvv=is.readLine();
                 os.println(eo.pago(cantidad_pagar,tarjeta,cvv));//escribimos en el socket lo recibido mas mensaje
                 os.flush();//limpiamos el socket
                os.close();
                is.close();
                clientSocket.close();
                System.out.println("Cliente Desatendido");
            }
            serverSocket.close();
        } catch (IOException e) {
